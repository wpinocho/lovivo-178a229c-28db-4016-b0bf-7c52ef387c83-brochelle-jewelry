...
export const ProductCardUI = ({ product }: ProductCardUIProps) => {
  return (
    <HeadlessProductCard product={product}>
      {(logic) => (
        <Card className="bg-card border border-border transition-luxury hover:shadow-lg hover:-translate-y-1">
          <CardContent className="p-4">
            <Link to={`/products/${logic.product.slug}`} className="block group">
                      ...
                      {opt.values.filter(val => logic.isOptionValueAvailable(opt.name, val)).map((val) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => logic.handleOptionChange(opt.name, val)}
                            className={`border rounded px-2 py-1 text-xs font-medium transition-luxury ${
                              isSelected 
            ...
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground transition-luxury disabled:opacity-50"
              >
